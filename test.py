...
class Map:
    ...
    def clear_other(self,queued=False,edges=False):
    ...
    def init_random_edges(self):
        ...
        for node in self.nodes:
            ...
            no_connected = random.randint(*self.connections)
            i=0
            for no in sorted(node.distances.items(), key=lambda x: x[1]):
    ...
    def find_solution(self):
        ...
        # get start node
        startNode = [x for x in self.nodes if x.position == "start"][0]
        endNode = [x for x in self.nodes if x.position == "end"][0]
    ...
    def get_route(self, node=None, routes_to=[]):
        ...
        if not node:
            node = [x for x in maps.nodes if x.position == "end"][0]
    ...
    def draw_nodes(self):
        ...
        for node in self.nodes:
            ...
            if node.queued:
                self.board_drawn = cv2.circle(self.board_drawn, (node.y, node.x), radius=int(self.w * size), color=(224, 145, 24), thickness=3)
            self.board_drawn = cv2.circle(self.board_drawn, (node.y, node.x), radius=int(self.w * size), color=colour, thickness=-1)
        ...
        for node in self.nodes:
            ...
            for join in node.connected:
                ...
                if not [node.ident, join[0].ident] in drawn:
                    self.board_drawn = cv2.line(self.board_drawn, node_pos, join[0].get_position(), color=(120, 120, 120), thickness=1)
    ...
    def draw_solution(self):
        ...
        for node in range(len(self.route)):
            if node != len(self.route) - 1:
                self.board_drawn = cv2.line(self.board_drawn, self.route[node].get_position(), self.route[node + 1].get_position(),
                                          color=(63, 224, 38),
                                          thickness=2)
...
def check_window(window):
    ...
    if event == "clear_edges":
        maps.clear_other(queued=True, edges=True)
        maps.route = []

    if event == "reload_edges":

        maps.clear_other(queued=True)
...
actions = {"init":True, "find":True, "draw":None}

while True:
    # get user defined board
    if actions["init"]:
        actions["init"] = False
